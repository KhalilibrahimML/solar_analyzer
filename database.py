...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        mock_data = [
            ...
            ('2026-06-03', 4.5, 6.2, 9.5),   # Underperforming run (e.g. dusty/soiled panels)
            ('2026-06-04', 5.5, 13.1, 10.5),
            ('2026-06-05', 5.1, 12.2, 9.9),
            ('2026-06-06', 1.8, 3.8, 9.0)    # Energy deficit run (stormy weather/heavy load override)
        ]
        cursor.executemany("""
...
def get_all_appliances():
    ...
    conn.close()
    return [{'id': r[0], 'name': r[1], 'power': r[2], 'hours': r[3], 'quantity': r[4]} for r in rows]

def get_generation_logs():
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT date, irradiance, actual_generation_kwh, load_consumed_kwh FROM generation_logs ORDER BY date ASC")
    rows = cursor.fetchall()
    conn.close()
    return rows